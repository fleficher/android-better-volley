...
public class BasicNetwork implements Network {
	...
	public BasicNetwork(HttpStack httpStack) {
		...
		// give us a lot of benefit and not use too much memory.
	...
	public NetworkResponse performRequest(Request<?> request)
		...
		while (true) {
			...
			try {
				...
				if (statusCode == HttpResponse.SC_NOT_MODIFIED) {
					...
					if (entry == null) {
						return new NetworkResponse(
								HttpResponse.SC_NOT_MODIFIED, null, null,
					...
					return new NetworkResponse(HttpResponse.SC_NOT_MODIFIED,
							entry.data, null, entry.responseHeaders, true,
							SystemClock.elapsedRealtime() - requestStart);
				}
				...
				return new NetworkResponse(statusCode, responseContents, responseInput,
				...
				if (responseContents != null || responseInput != null) {
					networkResponse = new NetworkResponse(statusCode,
							responseContents, responseInput, responseHeaders, false,
							SystemClock.elapsedRealtime() - requestStart);
